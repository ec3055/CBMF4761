...
def gae_for(args):
    ...
    # df = expand_adj_matrix.expand_adj_matrix(base_dir + "/Adjacency_matrix/{}_NEW.txt".format(sample_type))
    
    adj_df_ = pd.read_csv(base_dir + "/Adjacency_matrix/{}_NEW.txt".format(sample_type), sep="\t")
    adj_df = adj_df_.values[:,1:]
    adj = sp.csr_matrix(adj_df.astype(int))
    ...
    df = df.reindex(range(adj_df.shape[0]), fill_value=0)
    df = df.values
