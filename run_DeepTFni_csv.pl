...
sub run_DeepTFni{				
	...
	open (f1,"/content/drive/MyDrive/deeptfni/run_template_csv.sh") || die "Error";
	open (o1,">/content/drive/MyDrive/deeptfni/run_$sample_name.sh") || die "Error";
	while (<f1>)
		{
			my $s1 = substr($_,0,6);
			my $s2 = substr($_,0,9);
	...
	close f1;
	close o1;	
	system("sh /content/drive/MyDrive/deeptfni/run_$sample_name.sh") == 0 or die "Error in running run_$sample_name.sh\n";	
}
